...
for (rowData = 1; rowData <= findTestData('HitungProperti/TDHitungProperti').getRowNumbers(); rowData++) {
	...
	long tahunPemasukan = Long.parseLong(findTestData('HitungProperti/TDHitungProperti').getValue('Tahun', rowData))
    ...
    if ((pemasukan != '') && (pengeluaran != '')) {
       ...
       WebUI.selectOptionByValue(findTestObject('Page Hitung Harga Properti Maksimal BTN/dropdownTahun'), tahunPemasukan.toString(),
			false)
        ...
        if (tpTestCase == 'Positive') {
            ...
            long expectedHitung = ((pemasukan - pengeluaran) * (tahunPemasukan * 12)) / 3
			...
			WebUI.scrollToElement(findTestObject('Page Hitung Harga Properti Maksimal BTN/txtPemasukan'), 30)
			
            try {
                assert actualTextValidasi == 'Isi kurang dari nilai sebelumnya'
				
				WebUI.takeScreenshot(('D:\\' + nameCapture) + ' - success 01.png')
				...
				try {
					...
					WebUI.takeScreenshot(('D:\\' + nameCapture) + ' - success 02.png')
				}
				catch (Exception e) {
					WebUI.takeScreenshot(('D:\\' + nameCapture) + ' - failed.png')
				}
